...
export function KanbanColumn({ status, count, children }: KanbanColumnProps) {
  ...
  return (
    <section
      ...
      role="region"
      aria-label={`${config.label} column, ${count} items`}
    >
      <div className="flex items-center justify-between border-b border-accent-cyan/20 pb-2">
        ...
        >
          <span className={cn("block h-2 w-2", config.dotClass)} aria-hidden="true" />
          {config.label}
        </h2>
      ...
      <div className="flex flex-col gap-4" role="list">{children}</div>
    </section>
  );
}
