...
import type { HomeSummary, ReportBrief, ReportItem, ToolItem } from "./types";
        ...
        {expanded && (
          ...
          >
            <div className="px-5 pb-5">
              {item.brief && (item.brief.top_picks.length > 0 || item.brief.top_tools.length > 0 || item.brief.ai_signal) ? (
                <BriefQuadrants brief={item.brief} />
              ) : (
                /* Legacy fallback for historical reports without structured brief */
...
/** Shared 2x2 grid renderer for structured brief data (Intel Brief + report cards). */
function BriefQuadrants({ brief }: { brief: ReportBrief }) {
  const hasContent =
    brief.top_picks.length > 0 ||
    brief.top_tools.length > 0 ||
    brief.blog_ideas.length > 0 ||
    brief.ai_signal;
      ...
      {brief.top_picks.length > 0 && (
        <div className="space-y-2">
          ...
          <ul className="space-y-1.5">
            {brief.top_picks.slice(0, 3).map((pick, i) => (
...
function ResearchFeed() {
  ...
  // Stable key-based expansion: "intel-brief", report file_path, or null.
  const [expandedKey, setExpandedKey] = useState<string | null>("intel-brief");
      ...
      {(isLoading && !data) || summaryLoading ? (
        ...
        <div className="space-y-4 max-h-[700px] overflow-y-auto p-2 -m-2">
          {summary && (
            <IntelBriefCard
              summary={summary}
              expanded={expandedKey === "intel-brief"}
              onToggle={() => handleToggle("intel-brief")}
            />
          )}
          {data?.slice(0, 10).map((item) => {
            const key = item.file_path || `${item.type}-${item.date}`;
            return (
              <ResearchFeedCard
                key={key}
                item={item}
